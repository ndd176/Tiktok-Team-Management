    ...
    <div class="shop-detail">
      ...
      <div class="shop-card" *ngIf="shop">
        ...
        <div class="shop-info">
          ...
          <div class="shop-meta">
            <span class="badge" [class]="'badge-' + shop.status">{{shop.status}}</span>
            <span class="category">Category: {{shop.category}}</span>
            <span class="created-date">Created: {{shop.createdAt | date:'mediumDate'}}</span>
          </div>
        </div>
      </div>
      ...
      <div class="stats-section">
        ...
        <div class="stats-grid">
          <div class="stat-item">
            <div class="stat-number">{{shopTasks.length}}</div>
      ...
      <div class="tasks-section">
          ...
          <div 
            ...
            *ngFor="let task of filteredTasks"
            ...
            <div class="task-meta">
              ...
              <span class="assigned-to">Assigned to: {{task.assignedTo.name}}</span>
        ...
        <div class="no-tasks" *ngIf="filteredTasks.length === 0">
          <p>No tasks found for this shop with the selected filter.</p>
        </div>
      </div>
    </div>
  `,
...
export class ShopDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.loadShopTasks(shopId);
  }

  loadShopData(shopId: number) {
  ...
  loadShopTasks(shopId: number) {
    this.taskService.getTasks().subscribe(tasks => {
      this.shopTasks = tasks.filter(task => task.shop?.id === shopId);
      this.calculateStats();
      this.filterTasks();
    });
  }
  ...
  calculateStats() {
    this.completedTasks = this.shopTasks.filter(task => task.status === 'completed').length;
    this.inProgressTasks = this.shopTasks.filter(task => task.status === 'in-progress').length;
    this.pendingTasks = this.shopTasks.filter(task => task.status === 'pending').length;
    
    this.completionRate = this.shopTasks.length > 0 
      ? Math.round((this.completedTasks / this.shopTasks.length) * 100)
      : 0;

    const progressTasks = this.shopTasks.filter(task => task.progress > 0);
  ...
  filterTasks() {
    if (this.taskFilter === 'all') {
      this.filteredTasks = [...this.shopTasks];
    } else {
      this.filteredTasks = this.shopTasks.filter(task => task.status === this.taskFilter);
    }
  }
