...
router.get("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch channel", details: error.message });
  }
});
...
router.post("/", validateChannel, async (req, res) => {
  try {
    ...
    const existingChannel = await pool.query("SELECT id FROM channels WHERE name = $1", [name]);
...
router.put("/:id", validateChannel, async (req, res) => {
  try {
    ...
    const channelExists = await pool.query("SELECT id FROM channels WHERE id = $1", [id]);
