...
router.post("/", validateShop, async (req, res) => {
  try {
    ...
    const existingShop = await pool.query("SELECT id FROM shops WHERE name = $1", [name]);
...
router.put("/:id", validateShop, async (req, res) => {
  try {
    ...
    const shopExists = await pool.query("SELECT id FROM shops WHERE id = $1", [id]);
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to delete shop", details: error.message });
  }
});
